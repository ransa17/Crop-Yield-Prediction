...
def get_recommendation_from_gemini(prompt: str):
    """
    Generates a human-readable recommendation from a single prompt.
    """
...
def get_weekly_weather_data(region: str):
    """Simulates fetching average weekly weather data for a given region."""
    logging.info(f"Fetching average weekly weather data for {region}...")
    weather_data = {
        'North': {'rainfall_mm': 25, 'avg_temp_c': 20, 'humidity_percent': 70},
        'South': {'rainfall_mm': 5, 'avg_temp_c': 30, 'humidity_percent': 85},
        'East': {'rainfall_mm': 50, 'avg_temp_c': 25, 'humidity_percent': 90},
        'West': {'rainfall_mm': 10, 'avg_temp_c': 35, 'humidity_percent': 60},
    }
    return weather_data.get(region, {'rainfall_mm': 0, 'avg_temp_c': 0, 'humidity_percent': 0})
...
async def predict_yield(request: PredictionRequest):
    ...
    try:
        avg_weekly_weather = get_weekly_weather_data(request.region)
        ...
        input_data = pd.DataFrame([{
            ...
            'humidity_percent': avg_weekly_weather['humidity_percent'],
        ...
        weather_summary = f"rainfall: {avg_weekly_weather['rainfall_mm']}mm, temp: {avg_weekly_weather['avg_temp_c']}°C, humidity: {avg_weekly_weather['humidity_percent']}%"
        ...
        irrigation_rec = get_recommendation_from_gemini(irrigation_prompt)
        fertilizer_rec = get_recommendation_from_gemini(fertilizer_prompt)
        pest_control_rec = get_recommendation_from_gemini(pest_control_prompt)
        summary_rec = get_recommendation_from_gemini(summary_prompt)

        return {
            "predicted_yield": float(predicted_yield),
            "irrigation_rec": str(irrigation_rec),
            "fertilizer_rec": str(fertilizer_rec),
            "pest_control_rec": str(pest_control_rec),
